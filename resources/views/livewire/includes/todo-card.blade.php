            ...
            <div wire:key="{{$todo->id}}" class="todo mb-5 card px-5 py-6 bg-white col-span-1 border-t-2 border-blue-500 hover:shadow">
                <div class="flex justify-between space-x-2">
                    <div class="flex items-center">
                        @if($todo->completed)
                            <input wire:click="toggleCheck({{$todo->id}})" class="mr-2" type="checkbox" checked name="completed">


                             @if($updateTodoID==$todo->id)
                         <div>
                         <input wire:model="updateTodoName" type="text" placeholder="Todo.."
                                    class="bg-gray-100  text-gray-900 text-sm rounded block w-full p-2.5">
                                @error('updateTodoName')
                        ...
                        @endif    
                        @else
                            <input wire:click="toggleCheck({{$todo->id}})" class="mr-2" type="checkbox" name="completed">
